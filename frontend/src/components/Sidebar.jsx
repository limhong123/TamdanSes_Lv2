...
const links = {
  ...
  parent: [
    ...
    ["Result", "/parent/result", Award],
    ["Homework", "/parent/homework", BookOpen],
    ["Scores", "/parent/scores", FileText],
    ["Schedules", "/parent/schedules", CalendarDays],
    ["Notifications", "/parent/notifications", Bell],
    ["Profile", "/parent/profile", Users],
  ],
};
    ...
    <aside className="fixed left-0 top-0 h-screen w-64 border-r border-slate-200 bg-white">
      ...
      <nav className="space-y-1 p-4">
        ...
        <button
          type="button"
          onClick={logout}
          className="mt-4 flex w-full items-center gap-3 rounded-xl px-4 py-3 text-sm font-medium text-slate-600 hover:bg-slate-100"
        >
          <LogOut size={19} />
          Logout
        </button>
